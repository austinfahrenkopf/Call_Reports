...
function _walkFC(nodes,parts,sch){for(const nd of nodes){if(!nd.placeholder&&!nd.derived&&!nd.header&&nd.code&&!/^(H:|SEC:|SECBAR:|SUB:|EMPTY:)/.test(nd.code)){const cap=nd.caption||'';const anc=parts.filter(Boolean);if(!_fullCap.has(nd.code))_fullCap.set(nd.code,anc.length?anc.join(' — ')+' — '+cap:cap);}if(nd.header&&nd.code){const _sn=sch&&SCHED_NAMES[sch]?SCHED_NAMES[sch]:(parts.length?parts[0]:'');const _si=_sn.indexOf(' — ');const _sk=_si>=0?_sn.slice(0,_si):_sn;const _agg=sch?SUB_AGG_DESCS[sch]||'':'';const _cnt=(function c(n){let k=0;for(const x of(n.children||[])){if(x.header)k+=c(x);else if(x.code&&!x.placeholder&&!/^(H:|SEC:|SECBAR:|SUB:|EMPTY:)/.test(x.code))k++;}return k;})(nd);if(_cnt>0){const _rl=_agg?_sk+' '+_agg+': '+(nd.caption||''):_sk?_sk+' '+(nd.caption||''):(nd.caption||'');if(!_fullCap.has('SUB:'+nd.code))_fullCap.set('SUB:'+nd.code,_rl);if(!/^(H:|SEC:|EMPTY:)/.test(nd.code)&&!_fullCap.has(nd.code))_fullCap.set(nd.code,_rl);}}if(nd.children&&nd.children.length)_walkFC(nd.children,nd.header?[...parts,nd.caption||'']:parts,sch);}}
...
function rowEl(nd,has,dispCap){
 ...
 d.style.paddingLeft=(6+(nd.depth-1)*14)+'px';
 const car=`<span class="caret"${has?'':' style="visibility:hidden"'}>▸</span>`;
 // C18: filing-metadata / admin codes carry their instruction-sourced explanation on hover
 const _hovTxt=META_CODES[nd.code]||META_HOVER[nd.code]||nd.caption||'';
 const cap=`<span class="cap" title="${String(_hovTxt).replace(/"/g,'&quot;')}">${dispCap||nd.caption||''}</span>`;
...
function subDefFor(nd){
 ...
 if(r&&r.total){
   const comps=codes.filter(t=>t!==r.total);
...
async function buildReport(entityId,nm,latestQ,qtrs){
 ...
 const kpis=[
  ...
  {lbl:'NPL Ratio',val:fP(nplRat),betterWhen:'lower',spk:sparkline(qtrs.map(q=>[q,nplQ(q)]),true,COLORS[6]),qoq:nplQoQ,yoy:null,pc:null,
